...
主题中心后台默认使用经过整体验证的官方主题。每套官方主题固定字体、按钮、素材、动效和导航 recipe，管理员只选择主题并在安全范围内调整品牌强调色；保存后可直接打开用户前端查看真实内容效果，不再维护一套容易失真的后台手机预览。

主题导入 API 仍兼容已有外部主题数据：
...
PWA 安装提示属于站点行为和文案，由 **Site Settings → PWA 安装** 管理；它不属于视觉主题，切换主题时会保留当前安装提示设置。
