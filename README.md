...
一个基于 Astro SSR 与 Cloudflare Workers、D1、R2 构建的通用产品目录与转化站模板。公共站点采用面向美国市场的中性电商视觉，根路径直接进入默认或第一个已发布分区；分区首页、产品目录与产品详情是核心内容，通过中文管理后台维护站点、分区、分类、产品、Hero 图片广告、转化池及图片资产。

## 当前状态

项目按空数据模板维护，可直接配置为服务、资源、产品、内容或本地商家目录站：
...
Hero 广告保持简单模型：**图片 + 跳转链接**。按钮和广告文案直接制作在图片中，系统不维护额外广告标题或按钮文案，也不增加精选产品或最新产品模块。
...
公共站点使用中性白底、深灰文字、克制边框和系统字体，借鉴美国主流电商平台的清晰搜索、图片优先、信息分层与可预期交互，但不复制任何平台品牌。页面结构仍保持 Hero、分类入口、产品目录、产品详情和转化，不增加价格、评分、购物车、订单或推荐模块。

响应式层级：

- 手机端 `<768px`：双列产品、紧凑 Header、固定分区导航和固定详情 CTA
- 平板端 `768–1099px`：三列产品、独立平板宽度与单列详情构图
- PC 端 `≥1100px`：宽屏容器、四列产品、真正的左右详情布局
- 宽屏 PC `≥1400px`：五列产品目录
