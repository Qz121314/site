...
>
> 用户端支持 **English / Español**，管理后台使用中文。项目优先运行在 Cloudflare 免费计划上，并通过静态化、R2 公开快照、CDN 缓存和轻量 API 设计，尽可能减少 Worker 与 D1 的动态消耗。
...
- 分类数量较多时采用分层选择、搜索或侧边筛选，不依赖单一超长下拉框；
- PC 产品列表默认一行两列；
- 移动端根据屏幕宽度自适应；
- 产品卡片不显示独立缩略图；
...
| Static Assets | 静态资源请求免费且不限量；单版本最多 20,000 个文件 | React、JS、CSS、字体和静态页面壳 |
...
```text
浏览器
├─ 应用 JS/CSS/HTML ──────> Workers Static Assets
...
├─ apps/
│  ├─ web/
│  │  ├─ src/public/          英语/西班牙语用户端
│  │  ├─ src/admin/           中文管理后台
│  │  └─ src/messages/        统一消息中心 UI
│  └─ worker/
│     └─ src/
│        ├─ routes/
│        ├─ services/
...
1. **先做正确的数据边界，再做 UI。**
2. **公开读取静态化，后台写入动态化。**
3. **D1 是主数据源，R2 是公开发布层。**
4. **数据库只保存媒体对象 Key。**
5. **分类、频道、标签和转化方式不得写死。**
6. **客服是第三方系统，主项目只保留标准接口。**
7. **不为兼容第三方而降低自研客服的高级能力。**
8. **高频事件不写 D1。**
9. **免费额度不足时先优化和降级，再升级套餐。**
10. **安全、隐私和法律合规优先于功能数量。**

---

本 README 是当前项目的架构基线和开发计划。后续如修改核心业务规则、Cloudflare 资源边界、客服协议或数据生命周期，应同步更新本文档。