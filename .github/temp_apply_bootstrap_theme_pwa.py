...
# Worker: fold live theme settings into the existing bootstrap runtime D1 query.
path = "apps/worker/src/routes/public-storefront-config.ts"
replace(
    path,
...
replace(path, "  return { mediaBaseUrl, bottomNavigation };", "  return { mediaBaseUrl, theme, bottomNavigation };")
replace(
    path,
...
# Storefront content transport: keep this directly loadable by Node strip-types.
path = "apps/storefront/src/content.ts"
replace(
    path,
...
replace(path, "        resolveMediaBaseUrl(signal),\n        loadBottomNavigation(signal),\n      ]);", "        resolveMediaBaseUrl(signal),\n        loadBottomNavigation(signal),\n        loadTheme(signal),\n      ]);", 1)
replace(path, "    bottomNavigation,\n    sectionSnapshots,", "    theme,\n    bottomNavigation,\n    sectionSnapshots,", 1)
replace(
    path,
...
replace(path, "    configuredMediaBaseUrl: normalizeContentOrigin(\n      typeof value.mediaBaseUrl === 'string' ? value.mediaBaseUrl : null,\n    ),\n    bottomNavigation,", "    configuredMediaBaseUrl: normalizeContentOrigin(\n      typeof value.mediaBaseUrl === 'string' ? value.mediaBaseUrl : null,\n    ),\n    theme,\n    bottomNavigation,")
replace(
    path,
...
replace(path, "    home: normalizeV1HomeSnapshot(rawHome),\n    bottomNavigation,", "    home: normalizeV1HomeSnapshot(rawHome),\n    theme,\n    bottomNavigation,")

# Theme runtime: cached theme remains first-paint fallback; bootstrap theme becomes live authority.
path = "apps/storefront/src/theme-runtime.ts"
replace(
    path,
...
# Local PWA runtime store preserves immediate beforeinstallprompt listener mount.
Path("apps/storefront/src/pwa-install-runtime.ts").write_text(
...
path = "apps/storefront/src/main.tsx"
replace(path, "import { installStorefrontTheme } from './theme-runtime';", "import { installCachedStorefrontTheme } from './theme-runtime';")
replace(path, "const storefrontThemePromise = installStorefrontTheme();", "installCachedStorefrontTheme();")
replace(path, "      <PwaInstallPrompt themePromise={storefrontThemePromise} />", "      <PwaInstallPrompt />")

path = "apps/storefront/src/StorefrontRoot.tsx"
replace(path, "  Suspense,\n  useEffect,\n  useSyncExternalStore,", "  Suspense,\n  useEffect,\n  useLayoutEffect,\n  useSyncExternalStore,")
replace(path, "import { primaryNavigationItems } from './storefront-navigation';", "import { publishPwaInstallRuntime } from './pwa-install-runtime';\nimport { primaryNavigationItems } from './storefront-navigation';")
replace(path, "import { SYSTEM_UI } from './system-ui';", "import { SYSTEM_UI } from './system-ui';\nimport { applyStorefrontTheme } from './theme-runtime';")
replace(
    path,
...
path = "apps/storefront/src/PwaInstallPrompt.tsx"
replace(
    path,
...
# Storefront tests.
path = "apps/storefront/test/content.test.mjs"
replace(
    path,
...
replace(path, "        mediaBaseUrl: 'https://media.example.com',\n        bottomNavigation: bottomNavigation(),", "        mediaBaseUrl: 'https://media.example.com',\n        theme: storefrontTheme(),\n        bottomNavigation: bottomNavigation(),")
replace(path, "    if (url === '/api/public/storefront/media-base-url') {\n      return jsonResponse({ mediaBaseUrl: 'https://media.example.com' });\n    }", "    if (url === '/api/public/storefront/media-base-url') {\n      return jsonResponse({ mediaBaseUrl: 'https://media.example.com' });\n    }\n    if (url === '/api/public/theme') {\n      return jsonResponse({ theme: storefrontTheme() });\n    }")
replace(
    path,
...
replace(path, "    assert.equal(result.bottomNavigation[0].key, 'home');", "    assert.equal(result.bottomNavigation[0].key, 'home');\n    assert.equal(result.theme.key, 'saas');\n    assert.equal(result.theme.installPrompt.delaySeconds, 30);")
replace(path, "    assert.equal(bootstrap.bottomNavigation.length, 4);\n    assert.equal(bootstrap.bottomNavigation[2].key, 'messages');", "    assert.equal(bootstrap.bottomNavigation.length, 4);\n    assert.equal(bootstrap.bottomNavigation[2].key, 'messages');\n    assert.equal(bootstrap.theme.key, 'saas');\n    assert.equal(bootstrap.theme.installPrompt.delaySeconds, 30);")

# Worker tests: all rows carry the same site_settings theme fields.
for test_path in ["apps/worker/test/public-bootstrap-runtime.test.mjs", "apps/worker/test/modular-publication.test.mjs"]:
...
path = "apps/worker/test/public-bootstrap-runtime.test.mjs"
replace(path, "      assert.match(sql, /CROSS JOIN site_bottom_navigation nav/u);", "      assert.match(sql, /ss\\.theme_key/u);\n      assert.match(sql, /ss\\.theme_overrides_json/u);\n      assert.match(sql, /CROSS JOIN site_bottom_navigation nav/u);")
replace(path, "  assert.equal(runtime.mediaBaseUrl, 'https://media.example.com');", "  assert.equal(runtime.mediaBaseUrl, 'https://media.example.com');\n  assert.equal(runtime.theme.key, 'saas');\n  assert.equal(runtime.theme.installPrompt.delaySeconds, 30);")

path = "apps/worker/test/modular-publication.test.mjs"
replace(path, "import { publicStorefrontConfigRoutes } from '../src/routes/public-storefront-config.ts';", "import { publicStorefrontConfigRoutes } from '../src/routes/public-storefront-config.ts';\nimport { resolveTheme } from '../src/theme/theme-center.ts';")
replace(path, "      assert.match(sql, /CROSS JOIN site_bottom_navigation nav/u);", "      assert.match(sql, /ss\\.theme_key/u);\n      assert.match(sql, /ss\\.theme_overrides_json/u);\n      assert.match(sql, /CROSS JOIN site_bottom_navigation nav/u);", 1)
replace(path, "    mediaBaseUrl: 'https://media.example.com',\n    bottomNavigation,", "    mediaBaseUrl: 'https://media.example.com',\n    theme: resolveTheme({ key: 'saas', overrides: {} }),\n    bottomNavigation,")

Path("apps/storefront/test/storefront-bootstrap-theme-pwa.test.mjs").write_text(
...
path = "README.md"
replace(path, "→ 当前 mediaBaseUrl\n→ Bottom Navigation", "→ 当前 mediaBaseUrl\n→ 当前 Theme Runtime\n→ Bottom Navigation")
replace(
    path,
