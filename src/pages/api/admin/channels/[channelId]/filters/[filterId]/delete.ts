...
export const POST: APIRoute = async ({ request, params }) => {
  ...
  try {
    const result = await env.DB.prepare(
      "DELETE FROM category_filters WHERE id = ?1 AND channel_id = ?2",
    ).bind(filterId, channelId).run();

    if (result.meta.changes === 0) return redirect(request, channelId, { error: "not-found" });
  } catch (error) {
    console.error(JSON.stringify({ event: "admin_category_filter_delete_failed", channelId, filterId, error: String(error) }));
