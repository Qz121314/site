    ...
    const result = await env.DB.prepare(
      `DELETE FROM product_images
       WHERE product_id = ?1
         ...
         AND EXISTS (
           SELECT 1 FROM products p WHERE p.id = ?1 AND p.channel_id = ?3
         )`,
    ).bind(productId, imageId, channelId).run();

    if (!result.meta.changes) return redirect(request, channelId, productId, { error: "not-found" });
    return redirect(request, channelId, productId, { saved: "image-deleted" });
  } catch (error) {
    console.error(JSON.stringify({ event: "admin_product_image_delete_failed", channelId, productId, imageId, error: String(error) }));
