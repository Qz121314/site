...
test('package exposes the executable development contract', () => {
  for (const script of ['preflight:dev', 'fix:changed', 'precommit:check', 'verify', 'deploy:worker']) {
...
test('AGENTS points to preflight, exact-head CI, and classifier-owned release', () => {
  ...
  for (const doc of requiredDocs) assert.match(agents, new RegExp(doc.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
});

test('development docs have one owned structure', () => {
  for (const doc of requiredDocs) assert.equal(existsSync(doc), true, `${doc} must exist`);
...
test('development preflight cannot touch production Cloudflare', () => {
  ...
  assert.doesNotMatch(devPreflight, /--remote|wrangler d1|wrangler r2|wrangler deploy|CLOUDFLARE_API_TOKEN/);
});
