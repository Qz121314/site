...
test(
  'Messages promotional Article cards use bootstrap metadata and become read only after detail opens',
  async ({ page }) => {
    test.skip(
      !useLocalFixture,
    ...
    const articleRegion = page.getByRole('region', {
    ...
    expect(
      await page.evaluate(() =>
        localStorage.getItem('site:messages:read-articles'),
      ),
    ).toBe(null);
    ...
    await expect(returnedAlphaCard.getByText('Article', { exact: true })).toBeVisible();
...
test(
  'Messages with zero active Articles keeps the support conversation list as the only section',
  async ({ page }) => {
    test.skip(
      !useLocalFixture,
      'Messages Article acceptance uses the deterministic local fixture.',
