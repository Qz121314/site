...
test('Messages promotional Article cards use bootstrap metadata and become read only after detail opens', async ({
  page,
}) => {
  test.skip(!useLocalFixture, 'Messages Article acceptance uses the deterministic local fixture.');
  ...
  await expect(articleLinks.nth(1)).toContainText('A neutral card without background media.');
  await expect(page.getByText('Malformed placement')).toHaveCount(0);
  await expect(page.getByText('Conversations')).toBeVisible();
  await expect(page.getByRole('link', { name: /Support.*Support product/u })).toBeVisible();
  ...
  expect(await page.evaluate(() => localStorage.getItem('site:messages:read-articles'))).toBe(
    null,
  );
  ...
  await expect(page.getByRole('heading', { name: 'Alpha guide', level: 1 })).toBeVisible();
  await expect
    .poll(() =>
  ...
  await expect(returnedAlphaCard.getByText('Article')).toBeVisible();
  await expect(returnedAlphaCard.getByText('New')).toHaveCount(0);
  await expect(page.getByRole('link', { name: /Beta guide/u }).getByText('New')).toBeVisible();
...
test('Messages with zero active Articles keeps the support conversation list as the only section', async ({
  page,
}) => {
  test.skip(!useLocalFixture, 'Messages Article acceptance uses the deterministic local fixture.');
