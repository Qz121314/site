...
每次在线客服 CTA 由 `/go/{productId}` 生成唯一 `handoffId`。该编号随 Messages 临时会话创建请求直接传给 Customer Service，用于把**这一次 CTA handoff** 与客服系统的“首次接待”一一对应；它不代表 Site 侧流量统计，不写入 Site 侧流量账本，也不包含访客身份信息或增加额外 Worker 请求。
