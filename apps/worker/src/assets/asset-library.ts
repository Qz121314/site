    ...
    .prepare(
      `SELECT
         ...
         (
           SELECT COUNT(*)
           FROM sections s
           WHERE s.icon_asset_id = ma.id OR s.browse_background_asset_id = ma.id
      ...
      `UPDATE media_assets
       ...
       WHERE id = ?
         ...
         AND NOT EXISTS (
           SELECT 1
           FROM sections s
           WHERE s.icon_asset_id = media_assets.id OR s.browse_background_asset_id = media_assets.id
         )
