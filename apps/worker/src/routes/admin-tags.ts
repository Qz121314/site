...
function parseReorderItems(value: unknown): Array<{ id: string; sortOrder: number }> | null {
  if (!isRecord(value) || !Array.isArray(value.items) || value.items.length === 0 || value.items.length > MAX_BATCH_SIZE) {
    return null;
  }
...
async function requireSection(context: Parameters<typeof apiError>[0], sectionId: string) {
...
function tagDependencyError(context: Parameters<typeof apiError>[0], tag: ProductTagRecord) {
  return apiError(
    context,
    409,
...
adminTagRoutes.get('/:sectionId/tags', async (context) => {
  ...
  if (!scope) return apiError(context, 400, 'INVALID_PRODUCT_TAG_SCOPE', '标签列表范围无效。');
...
adminTagRoutes.post('/:sectionId/tags/batch-delete', async (context) => {
  context.header('Cache-Control', 'no-store');
  if (!hasAdminRequestHeader(context)) return apiError(context, 403, 'ADMIN_REQUEST_REQUIRED', '后台请求标识无效。');
  const sectionId = context.req.param('sectionId');
  const sectionError = await requireSection(context, sectionId);
  if (sectionError) return sectionError;
  const idempotencyKey = normalizeIdempotencyKey(context.req.header(IDEMPOTENCY_HEADER));
  if (!idempotencyKey) return apiError(context, 400, 'IDEMPOTENCY_KEY_REQUIRED', '批量删除缺少幂等键。');
  ...
  let body: unknown;
  try { body = await readJsonBody(context); } catch (error) { return jsonBodyError(context, error); }
  const ids = parseBatchIds(body);
  if (!ids) return apiError(context, 400, 'INVALID_PRODUCT_TAG_IDS', '请选择有效的标签。');
  const tags = await Promise.all(ids.map((id) => getProductTag(context.env.DB, sectionId, id)));
  const active = tags.filter((tag): tag is ProductTagRecord => Boolean(tag));
  if (active.length !== ids.length || active.some((tag) => tag.deletedAt)) return tagNotFound(context);
  ...
  for (const tag of active) {
    ...
    statements.push(
      ...
      createAuditLogStatement(context.env.DB, {
        action: 'product-tag.deleted', entityType: 'product_tag', entityId: tag.id,
        requestId: context.get('requestId'), before: { ...tag }, after: deleted,
        metadata: { sectionId, batch: true }, createdAt: now,
      }),
    );
  }
...
adminTagRoutes.post('/:sectionId/tags/reorder', async (context) => {
  ...
  if (sectionError) return sectionError;
  const idempotencyKey = normalizeIdempotencyKey(context.req.header(IDEMPOTENCY_HEADER));
  if (!idempotencyKey) return apiError(context, 400, 'IDEMPOTENCY_KEY_REQUIRED', '排序请求缺少幂等键。');
  const now = new Date().toISOString();
  ...
  if (!items) return apiError(context, 400, 'INVALID_PRODUCT_TAG_ORDER', '标签排序数据无效。');
  const tags = await Promise.all(items.map((item) => getProductTag(context.env.DB, sectionId, item.id)));
  if (tags.some((tag) => !tag || tag.deletedAt)) return tagNotFound(context);
  const responseBody = { reordered: true };
  const statements = items.map((item) => createReorderProductTagStatement(context.env.DB, sectionId, item.id, item.sortOrder, now));
  statements.push(
    createAuditLogStatement(context.env.DB, {
      action: 'product-tag.reordered', entityType: 'product_tag', requestId: context.get('requestId'),
      metadata: { sectionId, items }, createdAt: now,
    }),
...
adminTagRoutes.post('/:sectionId/tags', async (context) => {
  ...
  if (!validation.ok) return apiError(context, 400, 'INVALID_PRODUCT_TAG', validation.message, { field: validation.field });
  const now = new Date().toISOString();
  ...
  try {
    await context.env.DB.batch([
      created.statement,
      createAuditLogStatement(context.env.DB, {
        action: 'product-tag.created', entityType: 'product_tag', entityId: created.tag.id,
        requestId: context.get('requestId'), after: { ...created.tag }, metadata: { sectionId }, createdAt: now,
      }),
    ]);
  } catch (error) {
    if (isProductTagConflictError(error)) return apiError(context, 409, 'PRODUCT_TAG_NAME_CONFLICT', '当前分区已存在同名标签。');
    throw error;
  }
...
adminTagRoutes.put('/:sectionId/tags/:id', async (context) => {
  ...
  try {
    await context.env.DB.batch([
      createUpdateProductTagStatement(context.env.DB, sectionId, current.id, validation.value, now),
      createAuditLogStatement(context.env.DB, {
        action: 'product-tag.updated', entityType: 'product_tag', entityId: current.id,
        requestId: context.get('requestId'), before: { ...current }, after: { ...updated }, metadata: { sectionId }, createdAt: now,
      }),
    ]);
  } catch (error) {
    if (isProductTagConflictError(error)) return apiError(context, 409, 'PRODUCT_TAG_NAME_CONFLICT', '当前分区已存在同名标签。');
...
adminTagRoutes.delete('/:sectionId/tags/:id', async (context) => {
  ...
  await context.env.DB.batch([
    ...
    createAuditLogStatement(context.env.DB, {
      action: 'product-tag.deleted', entityType: 'product_tag', entityId: current.id,
      requestId: context.get('requestId'), before: { ...current }, after: deleted, metadata: { sectionId }, createdAt: now,
    }),
  ]);
...
adminTagRoutes.post('/:sectionId/tags/:id/restore', async (context) => {
  ...
  if (sectionError) return sectionError;
  const current = await getProductTag(context.env.DB, sectionId, context.req.param('id'));
  if (!current || !current.deletedAt) return apiError(context, 404, 'PRODUCT_TAG_NOT_FOUND', '回收站中不存在该标签。');
  const now = new Date().toISOString();
  ...
  try {
    await context.env.DB.batch([
      ...
      createAuditLogStatement(context.env.DB, {
        action: 'product-tag.restored', entityType: 'product_tag', entityId: current.id,
        requestId: context.get('requestId'), before: { ...current }, after: restored, metadata: { sectionId }, createdAt: now,
      }),
    ]);
  } catch (error) {
    if (isProductTagConflictError(error)) return apiError(context, 409, 'PRODUCT_TAG_RESTORE_CONFLICT', '当前分区已有同名标签，无法恢复。');
    throw error;
  }
