...
adminFaqRoutes.get('/', async (context) => {
  ...
  if (!scope) {
    return apiError(context, 400, 'INVALID_FAQ_SCOPE', 'FAQ 列表范围无效。');
  }

  return context.json({ faqs: await listFaqs(context.env.DB, scope) });
});

adminFaqRoutes.get('/:id', async (context) => {
  context.header('Cache-Control', 'no-store');
  const faq = await getFaq(context.env.DB, context.req.param('id'));
  if (!faq) return apiError(context, 404, 'FAQ_NOT_FOUND', 'FAQ 不存在。');
  return context.json({ faq });
});
...
adminFaqRoutes.post('/', async (context) => {
  ...
  try {
    ...
    if (isFaqConflictError(error)) {
      return apiError(context, 409, 'FAQ_QUESTION_CONFLICT', '已存在相同问题的 FAQ。');
    }
    throw error;
  }
...
adminFaqRoutes.post('/batch-delete', async (context) => {
  ...
  if (!ids) {
    return apiError(context, 400, 'INVALID_FAQ_IDS', '请选择有效的 FAQ。');
  }
...
adminFaqRoutes.post('/reorder', async (context) => {
  ...
  if (!items) {
    return apiError(context, 400, 'INVALID_FAQ_ORDER', 'FAQ 排序数据无效。');
  }
