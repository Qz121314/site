...
export function ProductEditorDialog({
  ...
  function toggleTag(tag: AdminProductTag) {
    const selected = form.tagIds.includes(tag.id);
    if (!selected && (!tag.isEnabled || form.tagIds.length >= 12)) return;
    patch({
      ...
      >
        ...
        <form className="product-editor-form" onSubmit={onSubmit}>
                ...
                {tags.length > 0 ? tags.map((tag) => {
                  ...
                  return (
                    <button
                      key={tag.id}
                      type="button"
                      className={`product-tag-option${selected ? ' is-selected' : ''}${!tag.isEnabled ? ' is-disabled' : ''}`}
                      disabled={busy || (!selected && (!tag.isEnabled || form.tagIds.length >= 12))}
                      onClick={() => toggleTag(tag)}
                    >
                      {tag.name}{!tag.isEnabled ? ' · 停用' : ''}
                    </button>
                  );
                }) : <span className="product-tags-empty">当前分区暂无标签</span>}
              </div>
            </div>
          ...
          <div className="product-body-field">
            ...
            {bodyMode === 'edit' ? (
              <textarea value={form.body} rows={12} maxLength={20_000} placeholder="输入产品介绍、服务内容和注意事项" onChange={(event) => patch({ body: event.target.value })} />
            ) : (
              <div className="product-markdown-preview">{form.body.trim() ? <MarkdownPreview source={form.body} /> : <p className="product-preview-empty">正文为空，暂无预览。</p>}</div>
            )}
          </div>
          ...
          <section className="product-media-section" aria-labelledby="product-media-title">
            <div className="product-media-heading">
              <div>
                ...
                <small>选择后在浏览器压缩为最长边 1400px、质量 0.82 的 WebP；可本地旋转、排序、删除和设封面。只有点击保存产品后才上传压缩图，原图不会上传到 R2。</small>
              </div>
            ...
            <div className="product-local-first-note"><strong>本地优先</strong><span>支持 JPG、PNG、WebP，最多 12 张。GIF 和 SVG 不上传原图，请先转换为静态图片。</span></div>
                  ...
                  return (
                    <article className={`product-media-card${isCover ? ' is-cover' : ''}${item.kind === 'local' ? ' is-local' : ''}`} key={item.key}>
                      <div className="product-media-preview">
                        {previewUrl ? <img src={previewUrl} alt={fileName} /> : <span>无公开预览</span>}
                        <div className="product-media-badges">{isCover ? <b>封面</b> : null}{item.kind === 'local' ? <b className="is-local-badge">本地待保存</b> : null}</div>
                      </div>
                      <div className="product-media-meta">
                        <strong title={fileName}>{fileName}</strong>
                        <small>{dimensions.width && dimensions.height ? `${dimensions.width} × ${dimensions.height}` : '尺寸未知'}{' · '}{formatImageBytes(getEditorImageByteSize(item))}</small>
                        {item.kind === 'local' ? <small>原图 {formatImageBytes(item.originalByteSize)}，仅保留在当前浏览器会话</small> : <small>已存储于 R2</small>}
                      </div>
                      <div className="product-media-actions">
                        ...
                        {item.kind === 'local' ? <><button type="button" disabled={busy} onClick={() => onRotateLocalImage(item.key, -1)}>左转</button><button type="button" disabled={busy} onClick={() => onRotateLocalImage(item.key, 1)}>{rotating ? '处理中…' : '右转'}</button></> : null}
                        <button type="button" disabled={isCover || busy} onClick={() => onSetCover(item.key)}>设为封面</button>
            ...
            {coverKey ? <button className="product-auto-cover" type="button" disabled={busy} onClick={() => onSetCover(null)}>改为自动使用第一张图片作为封面</button> : null}
          </section>
          ...
          <div className="product-form-grid product-publishing-grid">
            ...
            <label className="product-field"><span>产品排序</span><input type="number" min={0} max={1_000_000} value={form.sortOrder} onChange={(event) => patch({ sortOrder: Number(event.target.value) })} /></label>

            <label className="product-switch-field"><input type="checkbox" checked={form.isFeatured} onChange={(event) => patch({ isFeatured: event.target.checked })} /><span><strong>热门推荐</strong><small>启用后进入前端热门产品列表。</small></span></label>

            <label className="product-field"><span>热门排序</span><input type="number" min={0} max={1_000_000} value={form.featuredOrder} disabled={!form.isFeatured} onChange={(event) => patch({ featuredOrder: Number(event.target.value) })} /></label>
          </div>
