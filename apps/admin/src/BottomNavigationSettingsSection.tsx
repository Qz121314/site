...
export function BottomNavigationSettingsSection({
  ...
  const pickerItem = pickerKey ? value.find((item) => item.key === pickerKey) ?? null : null;

  function setIconType(key: BottomNavigationKey, iconType: BottomNavigationIconType) {
    const defaults: Record<BottomNavigationIconType, Pick<BottomNavigationItem, 'iconValue' | 'iconAssetId'>> = {
      builtin: { iconValue: key === 'browse' ? 'compass' : key === 'messages' ? 'messages' : key === 'faq' ? 'help' : 'home', iconAssetId: null },
  ...
  return (
    <section className="admin-settings-section" aria-labelledby="settings-bottom-navigation-title">
      <div className="admin-settings-section-heading">
        <div>
          ...
          <p className="admin-settings-section-description">
            路由固定，避免误配置；显示状态、名称和图标可以自由调整。图标支持内置 Icon、Emoji 和素材图片。
          </p>
        </div>
      </div>
      ...
      <div className="admin-bottom-navigation-grid">
        {value.map((item) => (
          <article className={`admin-bottom-navigation-card${item.enabled ? '' : ' is-disabled'}`} key={item.key}>
            <header className="admin-bottom-navigation-card-header">
              ...
              <label className="admin-bottom-navigation-switch">
                <input
                  ...
                  disabled={busy}
                  onChange={(event) => onChange(updateItem(value, item.key, { enabled: event.target.checked }))}
                />
            ...
            <div className="admin-bottom-navigation-fields">
              <label className="field-group">
                ...
                <input
                  ...
                  value={item.label}
                  disabled={busy}
                  onChange={(event) => onChange(updateItem(value, item.key, { label: event.target.value }))}
                />
              </label>
              ...
              <label className="field-group">
                ...
                <select
                  value={item.iconType}
                  disabled={busy}
                  onChange={(event) => setIconType(item.key, event.target.value as BottomNavigationIconType)}
                >
              ...
              {item.iconType === 'builtin' ? (
                <label className="field-group admin-bottom-navigation-icon-field">
                  ...
                  <select
                    ...
                    disabled={busy}
                    onChange={(event) => onChange(updateItem(value, item.key, { iconValue: event.target.value }))}
                  >
                    {BUILTIN_OPTIONS.map(([value, label]) => <option value={value} key={value}>{label}</option>)}
                  </select>
                </label>
              ) : null}
                  ...
                  <div className="admin-bottom-navigation-image-actions">
                    ...
                    {item.iconAssetId ? (
                      <button
                        ...
                        type="button"
                        disabled={busy}
                        onClick={() => onChange(updateItem(value, item.key, { iconAssetId: null }))}
                      >
                        移除
                      </button>
                    ) : null}
      ...
      {pickerKey && pickerItem ? (
        <MediaPickerDialog
          ...
          onSelect={(asset) => {
            onChange(updateItem(value, pickerKey, {
              iconType: 'asset',
              iconValue: null,
