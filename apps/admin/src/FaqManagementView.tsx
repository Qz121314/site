...
function sortFaqs(faqs: AdminFaq[]): AdminFaq[] {
  return [...faqs].sort((a, b) => a.sortOrder - b.sortOrder || b.updatedAt.localeCompare(a.updatedAt));
}
...
export function FaqManagementView({ onSessionExpired }: FaqManagementViewProps) {
  ...
  function openCreateEditor() {
    const sortOrder = activeFaqs.length ? Math.max(...activeFaqs.map((faq) => faq.sortOrder)) + 10 : 0;
    setEditingFaq(null);
  ...
  function openEditEditor(faq: AdminFaq) {
    setEditingFaq(faq);
    setForm({ title: faq.title, body: faq.body, sortOrder: faq.sortOrder, isEnabled: faq.isEnabled });
    setEditorMode('edit');
    setEditorOpen(true);
    setErrorMessage('');
    setSuccessMessage('');
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editingFaq) {
        ...
        setActiveFaqs((current) => sortFaqs(current.map((faq) => faq.id === updated.id ? updated : faq)));
  ...
  async function toggleEnabled(faq: AdminFaq) {
    ...
    try {
      ...
      setActiveFaqs((current) => current.map((item) => item.id === updated.id ? updated : item));
    ...
    <section className="faq-management" aria-label="FAQ 管理">
      ...
      {scope === 'active' && selectedIds.size > 0 ? (
        <div className="selection-toolbar">
          ...
          <button className="danger-button" type="button" disabled={working} onClick={() => setPendingDeleteIds([...selectedIds])}>批量删除</button>
        </div>
      ) : null}

      {loading ? (
        <div className="settings-card settings-loading"><div className="loading-indicator" /><p>正在读取 FAQ…</p></div>
      ) : filteredFaqs.length ? (
          ...
          {filteredFaqs.map((faq, index) => (
            <article className="faq-card" key={faq.id}>
              <div className="faq-card-select">
                {scope === 'active' ? <input type="checkbox" checked={selectedIds.has(faq.id)} onChange={() => toggleSelect(faq.id)} aria-label={`选择 FAQ ${faq.title}`} /> : null}
              </div>
              <div className="faq-card-content">
                <div className="faq-card-title-line">
                  <h3>{faq.title}</h3>
                  <button type="button" className={`status-pill ${faq.isEnabled ? 'is-enabled' : 'is-disabled'}`} disabled={scope === 'trash' || working} onClick={() => void toggleEnabled(faq)}>
                    {scope === 'trash' ? '已删除' : faq.isEnabled ? '已启用' : '已停用'}
                  </button>
                </div>
                <p>{bodySummary(faq.body)}</p>
                <small>排序 {faq.sortOrder} · 更新于 {formatDate(faq.updatedAt)}</small>
              </div>
              <div className="faq-card-actions">
                {scope === 'active' ? (
                  ...
                  <button type="button" disabled={working} onClick={() => void handleRestore(faq)}>恢复</button>
                )}
              </div>
            </article>
          ))}
        ...
        <div className="faq-empty-state"><strong>{search ? '没有符合条件的 FAQ' : scope === 'trash' ? '回收站为空' : '还没有 FAQ'}</strong></div>
      )}

      {editorOpen ? (
        <div className="admin-dialog-backdrop" role="presentation">
          <section className="admin-dialog faq-editor-dialog" role="dialog" aria-modal="true" aria-labelledby="faq-editor-title">
            <div className="admin-dialog-header">
              <div><p>FAQ 内容</p><h3 id="faq-editor-title">{editingFaq ? '编辑 FAQ' : '新增 FAQ'}</h3></div>
            ...
            <form className="faq-editor-form" onSubmit={(event) => void handleSave(event)}>
              ...
              <div className="faq-body-field">
                <div className="faq-body-label">
                  ...
                  <div className="faq-editor-tabs" role="tablist">
                    <button type="button" className={editorMode === 'edit' ? 'is-active' : undefined} onClick={() => setEditorMode('edit')}>编辑</button>
                    <button type="button" className={editorMode === 'preview' ? 'is-active' : undefined} onClick={() => setEditorMode('preview')}>预览</button>
                  </div>
                </div>
                {editorMode === 'edit' ? (
              ...
              <div className="admin-dialog-actions">
                <button type="button" className="secondary-button" disabled={saving} onClick={() => setEditorOpen(false)}>取消</button>
                <button className="primary-button" type="submit" disabled={saving}>{saving ? '正在保存…' : '保存 FAQ'}</button>
              </div>
            </form>
          </section>
        </div>
      ...
      {pendingDeleteIds.length > 0 ? (
        <div className="admin-dialog-backdrop" role="presentation">
          <section className="admin-dialog admin-dialog-small" role="alertdialog" aria-modal="true" aria-labelledby="faq-delete-title">
            <div className="admin-dialog-header"><div><p>删除 FAQ</p><h3 id="faq-delete-title">移入回收站？</h3></div></div>
            <p className="delete-warning">将 {pendingDeleteIds.length} 条 FAQ 移入回收站，可随后恢复。</p>
            <div className="admin-dialog-actions">
              <button type="button" className="secondary-button" disabled={working} onClick={() => setPendingDeleteIds([])}>取消</button>
              <button type="button" className="danger-button" disabled={working} onClick={() => void confirmDelete()}>{working ? '正在删除…' : '确认删除'}</button>
            </div>
          </section>
        </div>
      ) : null}
