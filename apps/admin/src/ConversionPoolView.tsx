...
export function ConversionPoolView({ section, onSessionExpired }: Props) {
  ...
  const filteredTargets = useMemo(() => {
    ...
    return keyword
      ? sourceTargets.filter((target) => targetSearchText(target).includes(keyword))
      : sourceTargets;
  ...
  const allTargetsSelected =
    ...
    filteredTargets.every((target) => selectedTargetIds.has(target.id));
  ...
  async function changeTargetScope(scope: Scope) {
    ...
    if (scope === 'trash') {
      ...
      try {
        setTrashTargets(
          await fetchConversionTargets(section.id, selectedGroup.id, 'trash'),
        );
      } catch (error) {
        handleError(error);
      } finally {
  ...
  async function saveGroup(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editingGroup) {
        const updated = await updateConversionGroup(
          section.id,
          editingGroup.id,
          groupForm,
        );
  ...
  async function confirmDelete() {
    ...
    try {
      if (deleteState.kind === 'group') {
        ...
        if (selectedGroupId && deleteState.ids.includes(selectedGroupId))
          setSelectedGroupId(null);
        setSelectedGroupIds(new Set());
        ...
        if (deleteState.ids.length === 1 && first) {
          ...
          await batchDeleteConversionTargets(
            section.id,
            selectedGroup.id,
            deleteState.ids,
          );
  ...
  return (
    <section className="conversion-pool" aria-labelledby="conversion-pool-title">
      ...
      {!groupEditorOpen && !targetEditorOpen && errorMessage ? (
      ...
      {rotationMessage ? (
        <div className="notice conversion-rotation-result" role="status">
          {rotationMessage}
        </div>
      ) : null}
      ...
      <div className="conversion-table-wrap ui-data-table-wrap">
        {groupsLoading ? (
          ...
          <AdminFeedbackState
            kind="empty"
            title="暂无转化分组"
            description="使用上方新增操作创建当前分区的转化分组。"
          />
        ) : (
          <table className="conversion-table ui-data-table">
            <thead>
              <tr>
                {groupScope === 'active' ? (
            ...
            <tbody>
              {filteredGroups.map((group, index) => (
                <tr
                  ...
                  aria-selected={
                    groupScope === 'active'
                      ? selectedGroupId === group.id || selectedGroupIds.has(group.id)
                      : undefined
                  }
                >
                  {groupScope === 'active' ? (
                  ...
                  <td>
                    <div className="conversion-group-identity">
                      <span
                        className={`conversion-mode-icon is-${group.mode}`}
                        aria-hidden="true"
                      >
                      ...
                      <div>
                        ...
                        <small>
                          {modeLabel(group)} · CTA: {group.buttonLabel}
                        </small>
                      </div>
                    </div>
                  </td>
          ...
          <AdminToolbar
            ...
            leading={
              <AdminSegmentedControl ariaLabel="链接状态">
                <AdminSegmentedItem
                  selected={targetScope === 'active'}
                  onClick={() => void changeTargetScope('active')}
                >
                  当前链接 {activeTargets.length}
                </AdminSegmentedItem>
                <AdminSegmentedItem
                  selected={targetScope === 'trash'}
                  onClick={() => void changeTargetScope('trash')}
                >
                  回收站 {trashTargets.length}
                </AdminSegmentedItem>
              </AdminSegmentedControl>
            }
            trailing={
              <>
                ...
                <Button variant="primary" onClick={openCreateTarget}>
                  添加链接
                </Button>
              </>
            }
          ...
          <div className="conversion-table-wrap">
            {targetsLoading ? (
              ...
              <AdminFeedbackState
                kind="empty"
                title="暂无链接"
                description="为当前链接分组添加第一个跳转地址。"
              />
            ) : (
              <table className="conversion-table conversion-target-table ui-data-table">
                <thead>
                  <tr>
                    {targetScope === 'active' ? (
                      <th className="checkbox-cell">
                ...
                <tbody>
                  {filteredTargets.map((target, index) => (
                    <tr
                      key={target.id}
                      className={`ui-data-row${selectedTargetIds.has(target.id) ? ' is-selected' : ''}`}
                      aria-selected={
                        targetScope === 'active'
                    ...
                    >
                      ...
                      <td>
                        {targetScope === 'active' ? (
                          ...
                          >
                            <AdminStatusBadge
                              tone={target.isEnabled ? 'success' : 'default'}
                            >
