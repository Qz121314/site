    ...
    return (
      <main className="admin-crash-screen" role="alert">
        <section className="admin-crash-card">
          <div className="admin-crash-mark" aria-hidden="true">!</div>
          <div className="admin-crash-copy">
            <p>后台运行异常</p>
            <h1>当前界面没有正常完成渲染</h1>
            <span>业务数据不会因为这个界面异常自动修改。可以先重试；如果仍然失败，返回站点设置重新进入模块。</span>
            {this.state.incidentId ? <code>错误编号 {this.state.incidentId}</code> : null}
          </div>
          <div className="admin-crash-actions">
            <button className="secondary-button" type="button" onClick={this.retry}>重试后台</button>
            <button className="primary-button" type="button" onClick={this.returnToSettings}>返回站点设置</button>
          </div>
        </section>
      </main>
    );
