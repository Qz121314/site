...
function isKey(value: unknown): value is BottomNavigationKey {
  return value === 'home' || value === 'browse' || value === 'messages' || value === 'faq';
}

function parseItem(value: unknown): BottomNavigationItemConfig | null {
  if (!isRecord(value) || !isKey(value.key) || typeof value.label !== 'string' || typeof value.enabled !== 'boolean') {
    return null;
  }
  if (!isRecord(value.icon)) return null;
  if (value.icon.type !== 'builtin' && value.icon.type !== 'emoji' && value.icon.type !== 'image') return null;
...
export async function loadBottomNavigation(signal?: AbortSignal): Promise<BottomNavigationItemConfig[]> {
  ...
  if (!isRecord(body) || !Array.isArray(body.items)) throw new Error('BOTTOM_NAVIGATION_INVALID');
