...
test(
  'schema-v2 bootstrap owns Hero data and resolves Hero media through runtime media config',
  async () => {
    ...
    globalThis.fetch = async (input) => {
      ...
      if (url === '/api/public/storefront/media-base-url') {
        return jsonResponse({
          mediaBaseUrl: 'https://media-runtime.example.com',
        });
      }
      ...
      if (
        url === `https://app.example.com/public/home/${POINTER_VERSION}/home.json`
      ) {
        return jsonResponse({
          schemaVersion: 2,
    ...
    try {
      ...
      assert.equal(
        requests.filter((url) =>
          url.endsWith(`/site/${SITE_VERSION}/site.json`),
        ).length,
        1,
      );
    } finally {
