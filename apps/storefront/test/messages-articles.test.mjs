...
test('bootstrap message article metadata is sanitized, sorted, deduplicated, and body-free', () => {
  ...
  assert.deepEqual(getMessageArticlesFromBootstrap(bootstrapWithMessageArticles(undefined)), []);
});
...
test('malformed and duplicate localStorage state recovers safely', () => {
  ...
  assert.deepEqual(
    [...readMessageArticleReadIds(memoryStorage('["article-a","article-a",4,null,"article-b"]'))],
    ['article-a', 'article-b'],
  );
});
...
test('messages badge composes independent support and article unread counts safely', () => {
  ...
  assert.equal(composeMessagesBadge(0, 2), 2, 'support reset must preserve article unread');
