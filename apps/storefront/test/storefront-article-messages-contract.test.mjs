...
test('Messages Article rows remain separate from support conversation data and only render on the list route', () => {
  ...
  assert.ok(messagesPage.includes("from './MessagesArticleList'"));
  ...
  assert.ok(messagesPage.includes('listHeader='));
  assert.ok(supportUi.includes('listHeader?: ReactNode'));
  assert.ok(supportUi.includes('{listHeader}'));
