...
test('route motion commits one live surface without browser snapshots', async () => {
  const [historyRuntime, navigationRuntime, presentationSource, rootSource, transitionStyles] =
    await Promise.all([
  ...
  assert.match(presentationSource, /flushSync\(\(\) => commitStorefrontLocation\('forward'\)\)/u);
  assert.match(presentationSource, /flushSync\(\(\) => commitStorefrontLocation\(direction\)\)/u);
