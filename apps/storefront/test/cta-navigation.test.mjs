...
test(
  'customer service CTA enters the chat shell before the handoff resolves',
  () => {
    ...
    assert.match(
      messagesPage,
      /parseResolvedComposePath\(path, composeContext\)/u,
    );
    assert.match(
      messagesPage,
    ...
    assert.match(
      supportUi,
      /loadingConversation && pendingConversation && !conversation/u,
    );
