...
test('mobile visitor composer separates focused controls from the keyboard and keeps controls prominent', () => {
  ...
  assert.ok(css.includes('border: 2px solid color-mix(in srgb, var(--brand) 54%, var(--line));'));
  assert.ok(css.includes('.chat-attachment-picker,'));
  assert.ok(css.includes('color: var(--brand-strong);'));
  assert.ok(css.includes('background: color-mix(in srgb, var(--brand) 12%, var(--surface));'));
