...
test(
  'production acceptance stays runtime-theme-aware and guards stable storefront contracts',
  async () => {
    ...
    assert.match(
      acceptance,
      /document\.documentElement\.dataset\.navigationStyle/u,
    );
    ...
    assert.match(
      home,
      /\.home-product-rail \{[\s\S]*grid-template-columns: repeat\(2,/u,
    );
    assert.match(
      home,
      /\.home-product-cover,[\s\S]*aspect-ratio: 1 \/ 1;/u,
    );
    assert.match(
      browse,
      /\.browse-section-card \{[\s\S]*aspect-ratio: 16 \/ 10;/u,
    );
    assert.match(
      browse,
    ...
    assert.match(
      section,
      /\.section-product-cover \{[\s\S]*aspect-ratio: 1 \/ 1;/u,
    );
  },
);
