...
test('Browse uses full-bleed image cards on mobile and desktop', () => {
  ...
  assert.match(
    browseCss,
    /\.browse-section-card-content\s*\{[^}]*position:\s*absolute[^}]*z-index:\s*3/su,
  );
  assert.doesNotMatch(
    browseCss,
    /\.browse-section-card\s*\{[^}]*grid-template-columns:\s*(?:96|104)px/su,
  );
  assert.match(
    browseCss,
