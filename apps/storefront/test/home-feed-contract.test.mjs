...
test('root path mounts the dedicated Home feed while other paths keep the existing App', () => {
  assert.match(mainSource, /<StorefrontRoot \/>/u);
  assert.match(rootSource, /pathname === '\/' \? <HomeRoot \/> : <App \/>/u);
...
test('Home is Hero then seven configured section shortcuts plus Browse More', () => {
  ...
  assert.match(feedSource, /<HomeShortcuts sections=\{shortcutSections\} moreLabel=\{copy\.home\.viewAll\} \/>/u);
...
test('Home layout is backend runtime config with seven and three item limits', () => {
  ...
  assert.match(layoutSource, /parseSectionIds\(body\.layout\.recommendationSectionIds, 3\)/u);
});
