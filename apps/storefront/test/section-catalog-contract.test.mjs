...
const cssSource = await readFile(new URL('../src/section-ui.css', import.meta.url), 'utf8');
...
test('section route uses a dedicated primary-shell product catalog', () => {
  ...
  assert.match(rootSource, /isSectionPath\(pathname\)/u);
  assert.match(rootSource, /<SectionRoot pathname=\{pathname\} \/>/u);
...
test('section header stays content-focused and does not reuse Home shortcut icons', () => {
  assert.match(sectionSource, /<h1 id="section-catalog-title">\{query\.data\.section\.name\}<\/h1>/u);
...
test('section products remain a two-column product list on mobile and desktop', () => {
  ...
  assert.match(cssSource, /\.section-catalog-products\s*\{\s*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/u);
  assert.match(cssSource, /@media \(max-width:\s*767px\)[\s\S]*\.section-catalog-products/u);
});
