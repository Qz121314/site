...
const messagesCss = await readFile(new URL('../src/messages-ui.css', import.meta.url), 'utf8');
...
test('conversation list is compact, newest-first, and keeps product context in the preview', () => {
  ...
  assert.match(supportSource, /conversationTimestamp\(right\) - conversationTimestamp\(left\)/u);
  ...
  assert.match(supportSource, /Yesterday/u);
});
...
test('desktop Messages uses a two-pane conversation and chat workspace', () => {
  ...
  assert.match(messagesCss, /@media \(min-width:\s*768px\)[\s\S]*\.chat-back-button\s*\{[\s\S]*display:\s*none/u);
});

test('mobile opens a conversation as a dedicated chat view without global chrome', () => {
  assert.match(messagesCss, /@media \(max-width:\s*767px\)[\s\S]*\.messages-workspace\.is-thread-open \.messages-sidebar\s*\{[\s\S]*display:\s*none/u);
  assert.match(shellCss, /@media \(max-width:\s*767px\)[\s\S]*\.app-shell:has\(\.messages-workspace\.is-thread-open\) > \.topbar/u);
