...
if (!(theme.headers.get('content-type') ?? '').includes('application/json'))
  throw new Error('Theme content-type contract failed');
if (
  (theme.headers.get('cache-control') ?? '') !== 'public, max-age=30, must-revalidate'
) {
  throw new Error('Theme cache-control contract failed');
}
const themePayload = await theme.json();
if (
  !themePayload?.theme?.key ||
  !['light', 'dark'].includes(themePayload.theme.colorScheme)
) {
...
if (authPayload.authenticated !== false)
  throw new Error('Anonymous Admin session boundary failed');
...
if (notFound.status !== 404)
  throw new Error(`Storefront not-found route returned HTTP ${notFound.status}`);
...
if (!notFoundBody.includes('class="boot-shell"'))
  throw new Error('Storefront not-found shell contract failed');
